...
async function createTestData() {
    ...
    if (!customerResponse.ok) throw new Error('Falha ao criar cliente: ' + await customerResponse.text());
    ...
    if (!deviceResponse.ok) throw new Error('Falha ao criar dispositivo: ' + await deviceResponse.text());
    ...
    if (!ticketResponse.ok) throw new Error('Falha ao criar ticket: ' + await ticketResponse.text());
    ...
    if (!timelineResponse.ok) throw new Error('Falha ao criar timeline: ' + await timelineResponse.text());
