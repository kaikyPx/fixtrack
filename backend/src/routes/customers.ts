...
router.get('/', async (req, res) => {
  try {
    const [rows] = await pool.execute(
      `SELECT id, name, phone, phone_type as phoneType, contact_name as contactName, 
...
router.post('/', async (req, res) => {
  try {
    ...
    await pool.execute(
      `INSERT INTO customers (id, name, phone, phone_type, contact_name, phone_secondary, 
                              phone_secondary_type, cpf, zip_code, email, contact_observations, observations) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, name, phone, phoneType || 'CLIENTE', contactName || null, phoneSecondary || null,
...
router.put('/:id', async (req, res) => {
  try {
    ...
    await pool.execute(
      `UPDATE customers SET name = ?, phone = ?, phone_type = ?, contact_name = ?, 
                            phone_secondary = ?, phone_secondary_type = ?, cpf = ?, 
                            zip_code = ?, email = ?, contact_observations = ?, observations = ? 
       WHERE id = ?`,
      [name, phone, phoneType || 'CLIENTE', contactName || null, phoneSecondary || null,
