...
router.post('/', async (req, res) => {
    try {
        ...
        await pool.execute(
            `INSERT INTO financial_transactions (id, type, category, amount, description, date, customer_id, ticket_id, status) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [id, type, category, amount, description || null, transactionDate, customerId || null, ticketId || null, status || 'PAGO']
        );

        // Log the creation
        if (userId && userName) {
            await pool.execute(
                `INSERT INTO financial_logs (id, user_id, user_name, action, transaction_details) VALUES (?, ?, ?, ?, ?)`,
                [
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const [rows]: any = await pool.execute('SELECT * FROM financial_transactions WHERE id = ?', [req.params.id]);
