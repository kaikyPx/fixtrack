...
router.get('/:id', async (req, res) => {
    try {
        ...
        UNIX_TIMESTAMP(t.created_at) * 1000 as createdAt,
        UNIX_TIMESTAMP(t.updated_at) * 1000 as updatedAt
      FROM tickets t WHERE t.id = ?`, [req.params.id]);
        const tickets = rows;
        if (tickets.length === 0) {
...
router.post('/move/:id', async (req, res) => {
    ...
    try {
        const { status, userId, userName, comment, technicianId, partName, forecastDate, cancellationReason, attachment, returnMethod, resolutionResult, repairWarrantyDays, finalObservations, deliveryConfirmed, signatureUrl, swapDeviceInfo, warrantyDenialInfo } = req.body;
        ...
        if (status === 'Pronto' || status === 'Entregue ao cliente' || status === 'Troca realizada') {
            ...
            queryParams.push(returnMethod || null, resolutionResult || null, repairWarrantyDays || 0, finalObservations || null, deliveryConfirmed ? 1 : 0, signatureUrl || null, swapDeviceInfo ? JSON.stringify(swapDeviceInfo) : null, warrantyDenialInfo ? JSON.stringify(warrantyDenialInfo) : null);
        }
        ...
        await connection.execute('INSERT INTO timeline_entries (id, ticket_id, user_id, user_name, action, description, attachment_url, attachment_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [timelineId, ticketId, userId, userName, `Status alterado para: ${status}`, fullDescription, attachment?.url || null, attachment?.name || null]);
        // 3. Se houver anexo, salvar na tabela attachments
        if (attachment) {
            const attId = `att_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
            await connection.execute('INSERT INTO attachments (id, ticket_id, name, type, url) VALUES (?, ?, ?, ?, ?)', [attId, ticketId, attachment.name || 'Anexo de Movimentação', attachment.type || 'image/jpeg', attachment.url]);
        }
