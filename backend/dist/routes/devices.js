...
router.get('/', async (req, res) => {
    try {
        const [rows] = await pool.execute(`SELECT id, customer_id as customerId, type, brand, model, imei_or_serial as imeiOrSerial, color, storage,
...
router.post('/', async (req, res) => {
    try {
        const { id, customerId, type, brand, model, imeiOrSerial, color, storage, serialNumber, screenPassword, batteryHealth, isReturn, accessories, observations, conditionOnArrival, supportEntryData, loanerDevice, mediaFiles, purchaseDate, warrantyPeriodMonths, warrantyEndDate, supplierId, supplierName, stockEntryDate, supplierWarrantyMonths, supplierWarrantyEndDate } = req.body;
        await pool.execute(`INSERT INTO devices (id, customer_id, type, brand, model, imei_or_serial, color, storage, serial_number,
...
router.put('/:id', async (req, res) => {
    try {
        const { type, brand, model, imeiOrSerial, color, storage, serialNumber, screenPassword, batteryHealth, isReturn, accessories, observations, conditionOnArrival, supportEntryData, loanerDevice, mediaFiles, purchaseDate, warrantyPeriodMonths, warrantyEndDate, supplierId, supplierName, stockEntryDate, supplierWarrantyMonths, supplierWarrantyEndDate } = req.body;
        await pool.execute(`UPDATE devices SET type = ?, brand = ?, model = ?, imei_or_serial = ?, color = ?, storage = ?, serial_number = ?,
        screen_password = ?, battery_health = ?, is_return = ?,
        condition_screen_ok = ?, condition_case_ok = ?, condition_camera_ok = ?, condition_impact_signs = ?, condition_liquid_damage_signs = ?,
        entry_date = ?, entry_method = ?, received_by = ?, priority = ?, estimated_deadline = ?,
        has_loaner_device = ?, loaner_model = ?, loaner_imei = ?, loaner_delivery_date = ?, liability_term = ?,
