...
router.get('/:ticketId', async (req, res) => {
    try {
      ...
      WHERE ticket_id = ? 
      ORDER BY timestamp DESC`, [req.params.ticketId]);
...
router.post('/', async (req, res) => {
    try {
        ...
        await pool.execute('INSERT INTO timeline_entries (id, ticket_id, user_id, user_name, action, description, attachment_url, attachment_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [id, ticketId, userId, userName, action, description, attachmentUrl || null, attachmentName || null]);
        res.status(201).json({
