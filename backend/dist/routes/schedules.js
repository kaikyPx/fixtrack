...
router.post('/', async (req, res) => {
    try {
        ...
        await pool.execute('INSERT INTO schedules (id, customer_id, ticket_id, type, method, scheduled_at, observations) VALUES (?, ?, ?, ?, ?, ?, ?)', [id, customerId, ticketId || null, type, method, scheduledAtValue, observations || null]);
        res.status(201).json({
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const scheduledAtValue = scheduledAt ? new Date(scheduledAt).toISOString().slice(0, 19).replace('T', ' ') : null;
        await pool.execute('UPDATE schedules SET type = ?, method = ?, scheduled_at = ?, status = ?, observations = ? WHERE id = ?', [type, method, scheduledAtValue, status, observations || null, req.params.id]);
